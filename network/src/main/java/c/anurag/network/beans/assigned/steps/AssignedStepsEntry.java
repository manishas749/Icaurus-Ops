...
public class AssignedStepsEntry implements IEntry<AssignedStepsResponse> {
    private final String tableName;
    private final IcarusDatabaseManager icarusDatabaseManager;

    public AssignedStepsEntry(String tableName, IcarusDatabaseManager icarusDatabaseManager) {
        this.tableName = tableName;
        this.icarusDatabaseManager = icarusDatabaseManager;
    }

    @Override
    public AssignedStepsResponse insertUpdate(AssignedStepsResponse response) {
        List<AssignedSteps> data = response.getData();
        String insertSql = "INSERT INTO " + tableName + " (uuid, assigned_checklist_uuid, " +
                "step_id, checklist_element_id, status, user_id, is_deleted, created, modified, " +
                " sync_status) VALUES (?,?,?,?,?,?,?,?,?,?);";
        String updateSql = "UPDATE " + tableName + " SET uuid = ?, " +
                "assigned_checklist_uuid = ?, step_id = ?, checklist_element_id = ?, status = ?, user_id = ?, " +
                "is_deleted = ?, created = ?, modified = ?,  " +
                "sync_status = ? WHERE assigned_checklist_uuid = ? AND step_id = ? AND checklist_element_id = ?" +
                "AND modified <= ?";
    ...
    private void bulkInsertUpdate(SQLiteStatement statement, AssignedSteps obj, boolean insert) {
        int count = 1;
        int syncStatus = 1;
