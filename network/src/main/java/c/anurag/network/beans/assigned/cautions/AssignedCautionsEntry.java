...
public class AssignedCautionsEntry implements IEntry<AssignedCautionsResponse> {
    private final String tableName;
    private final IcarusDatabaseManager icarusDatabaseManager;

    public AssignedCautionsEntry(String tableName, IcarusDatabaseManager icarusDatabaseManager) {
        this.icarusDatabaseManager = icarusDatabaseManager;
        this.tableName = tableName;
    }

    @Override
    public AssignedCautionsResponse insertUpdate(AssignedCautionsResponse response) {
        List<AssignedCautions> data = response.getData();
        String insertSql = "INSERT INTO " + tableName + " (uuid, assigned_checklist_uuid, " +
                "user_id, caution_id, checklist_element_id, acknowledged, is_deleted, created, modified, " +
                "sync_status) VALUES (?,?,?,?,?,?,?,?,?,?);";
        String updateSql = "UPDATE " + tableName + " SET uuid = ?, assigned_checklist_uuid = ?, " +
                "user_id = ?, caution_id = ?, checklist_element_id = ?, acknowledged = ?, is_deleted = ?, " +
                "created = ?, modified = ?,  sync_status = ? " +
                "WHERE assigned_checklist_uuid = ? AND caution_id = ?  AND checklist_element_id = ? AND modified <= ?";
        String updateStatusSql = "UPDATE " + tableName + " SET sync_status = ? WHERE assigned_checklist_uuid = ? AND caution_id = ? AND checklist_element_id = ?";
            ...
            try {
                if (!TextUtils.isEmpty(obj.getOperation())) {
                    ...
                    if (obj.getOperation().equalsIgnoreCase("insert") || obj.getOperation().equalsIgnoreCase("update")) {
                        ...
                        updateStatusStatement.bindLong(3, obj.getCautionId());
                    ...
                    String selectSql = "SELECT\n" +
                            ...
                            " AND caution_id = " + obj.getCautionId() +
    ...
    private void bulkInsertUpdate(SQLiteStatement statement, AssignedCautions obj, boolean insert) {
        int count = 1;
        int syncStatus = 1;
