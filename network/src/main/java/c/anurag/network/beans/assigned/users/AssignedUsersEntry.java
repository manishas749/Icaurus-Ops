...
public class AssignedUsersEntry implements IEntry<AssignedUsersResponse> {
    private final String tableName;
    private final IcarusDatabaseManager icarusDatabaseManager;

    public AssignedUsersEntry(String tableName, IcarusDatabaseManager icarusDatabaseManager) {
        this.tableName = tableName;
        this.icarusDatabaseManager = icarusDatabaseManager;
    }

    @Override
    public AssignedUsersResponse insertUpdate(AssignedUsersResponse response) {
        List<AssignedUsers> data = response.getData();
        String insertSql = "INSERT INTO " + tableName + " (uuid, assigned_checklist_uuid, " +
                "user_id, assigned_by, is_deleted, created, modified, " +
                "sync_status) VALUES (?,?,?,?,?,?,?,?);";
        String updateSql = "UPDATE " + tableName + " SET uuid = ?, " +
                "assigned_checklist_uuid = ?, user_id = ?, assigned_by = ?, is_deleted = ?, " +
                "created = ?, modified = ?,  sync_status = ? " +
                "WHERE assigned_checklist_uuid = ? AND user_id = ? AND modified <= ?";
        String updateStatusSql = "UPDATE " + tableName + " SET sync_status = ? WHERE assigned_checklist_uuid = ? AND user_id = ?";
            ...
            try {
                if (!TextUtils.isEmpty(obj.getOperation())) {
                    ...
                    if (obj.getOperation().equalsIgnoreCase("insert") || obj.getOperation().equalsIgnoreCase("update")) {
                        ...
                        updateStatusStatement.bindLong(3, obj.getUserId());
    ...
    private void bulkInsertUpdate(SQLiteStatement statement, AssignedUsers obj, boolean insert) {
        int count = 1;
        int syncStatus = 1;
        ...
        statement.bindLong(count++, obj.getAssignedBy());
