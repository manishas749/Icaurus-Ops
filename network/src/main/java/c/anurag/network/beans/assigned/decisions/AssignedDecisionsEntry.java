...
public class AssignedDecisionsEntry implements IEntry<AssignedDecisionsResponse> {

    private final String tableName;
    private final IcarusDatabaseManager icarusDatabaseManager;

    public AssignedDecisionsEntry(String tableName, IcarusDatabaseManager icarusDatabaseManager) {
        this.tableName = tableName;
        this.icarusDatabaseManager = icarusDatabaseManager;
    }

    public AssignedDecisionsResponse insertUpdate(AssignedDecisionsResponse response) {
        List<AssignedDecisions> data = response.getData();
        String insertSql = "INSERT INTO " + tableName + " (uuid, assigned_checklist_uuid, " +
                "decision_id, checklist_element_id, user_id, is_applicable, is_deleted, created, modified, " +
                " sync_status) VALUES (?,?,?,?,?,?,?,?,?,?);";
        String updateSql = "UPDATE " + tableName + " SET uuid = ?, " +
                "assigned_checklist_uuid = ?, decision_id = ?, checklist_element_id =?, user_id = ?, " +
                "is_applicable = ?, is_deleted = ?, created = ?, modified = ?, " +
                "  sync_status = ? WHERE " +
                "assigned_checklist_uuid = ? AND decision_id = ? AND checklist_element_id = ? AND modified <= ?";
        String updateStatusSql = "UPDATE " + tableName + " SET sync_status = ? WHERE assigned_checklist_uuid = ? " +
                "AND decision_id = ? AND checklist_element_id = ?";
        ...
        for (AssignedDecisions obj : data) {
            try {
                if (!TextUtils.isEmpty(obj.getOperation())) {
                    ...
                    if (obj.getOperation().equalsIgnoreCase("insert") || obj.getOperation().equalsIgnoreCase("update")) {
                        ...
                        updateStatusStatement.bindLong(3, obj.getDecisionId());
                    ...
                    String selectSql = "SELECT uuid FROM " + tableName + " WHERE assigned_checklist_uuid = '" + obj.getAssignedChecklistUuid() +
                            "' AND decision_id = " + obj.getDecisionId() + " AND checklist_element_id = " + obj.getChecklistElementId();
    ...
    private void bulkInsertUpdate(SQLiteStatement statement, AssignedDecisions obj, boolean insert) {
        int count = 1;
        int syncStatus = 1;
        statement.clearBindings();
        statement.bindString(count++, obj.getUuid());
        statement.bindString(count++, obj.getAssignedChecklistUuid());
        statement.bindLong(count++, obj.getDecisionId());
