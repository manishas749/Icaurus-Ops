 ...
 */
public class MyAssignedFragment extends ChecklistBaseFragment<FragmentChecklistBinding, DashboardViewModel> {
    ...
    @Override
    public void onMyAssignedChecklistClick(final MyAssignedChecklistItems item) {
        super.onMyAssignedChecklistClick(item);
        if (item != null && item.getChecklistSyncStatus() == ChecklistStatus.NOT_SYNCHRONIZED) {
                                ...
                                public void onChanged(@Nullable WorkInfo workStatus) {
                                    if (workStatus != null) {
                                        if (workStatus.getState() == WorkInfo.State.SUCCEEDED) {
                                            WorkManager.getInstance(getActivity()).getWorkInfoByIdLiveData(oneTimeWorkRequest.getId()).removeObservers(getViewLifecycleOwner());
                                            ...
                                            onMyAssignedChecklistClick(item);
                                            mDashboardViewModel.setIsLoading(false);
        ...
        } else if (item != null && !mDashboardViewModel.isUserExist(item.getUuid()) && item.getChecklistStatus().equals("New")) {
            if (mDashboardViewModel.getItemPlaceHolders(item.getUuid()).size() > 0)
                openPlaceholderList(item);
            else
                DialogUtility.showAlertWithCloseContinueOnly(getActivity(), getString(R.string.start_checklist_or_goback), 0, this, item, getString(R.string.assign_to_me));
        } else if (item != null && !mDashboardViewModel.isUserExist(item.getUuid()) && item.getChecklistStatus().equals("Paused")) {
            if (mDashboardViewModel.getItemPlaceHolders(item.getUuid()).size() > 0)
                openPlaceholderList(item);
            else
                DialogUtility.showAlertWithCloseContinueOnly(getActivity(), getString(R.string.resume_checklist_or_go_back), 0, this, item, getString(R.string.assign_to_me));
        } else if (item != null && item.getChecklistStatus().equals("New")) {
            if (mDashboardViewModel.getItemPlaceHolders(item.getUuid()).size() > 0)
                openPlaceholderList(item);
            else
                DialogUtility.showAlertWithCloseContinueOnly(getActivity(), getString(R.string.start_checklist_message), 0, this, item, "Start");
        } else if (item != null && item.getChecklistStatus().equals("Paused")) {
            if (mDashboardViewModel.getItemPlaceHolders(item.getUuid()).size() > 0)
                openPlaceholderList(item);
            else
                DialogUtility.showAlertWithCloseContinueOnly(getActivity(), getString(R.string.resume_checklist_message), 0, this, item, "Resume");
        } else if (item != null && item.getChecklistStatus().equals("In Progress")) {
            if (mDashboardViewModel.getItemPlaceHolders(item.getUuid()).size() > 0)
                openPlaceholderList(item);
            else
    ...
    public <T> void continueChecklist(T item) {
        mDashboardViewModel.updateMyAssignedChecklist((MyAssignedChecklistItems) item);
        openChecklistDetailScreen(((MyAssignedChecklistItems) item).getChecklistId(), ((MyAssignedChecklistItems) item).getUuid(), ((MyAssignedChecklistItems) item).getTitle(), ((MyAssignedChecklistItems) item).getIsSequential());
    }

    @Override
    public void onStartObserving() {

        mDashboardViewModel.observeMyAssignedCheckList().observe(getViewLifecycleOwner(), new Observer<PagedList<MyAssignedChecklistItems>>() {
            @Override
            public void onChanged(@Nullable PagedList<MyAssignedChecklistItems> myAssignedChecklistItems) {
                mDashboardViewModel.setIsLoading(false);
                mDashboardViewModel.setMyAssignedChecklistAdapter(myAssignedChecklistItems);
            }
        });
    }
    ...
    public void removeObserverIfAny() {
        if (mDashboardViewModel.observeMyAssignedCheckList() != null && mDashboardViewModel.observeMyAssignedCheckList().hasObservers())
            mDashboardViewModel.observeMyAssignedCheckList().removeObservers(getViewLifecycleOwner());
    }
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == Constants.REQUEST_RESULT_FOR_START_CHECKLIST && resultCode == RESULT_OK) {
            if (mDashboardViewModel.getMyAssignedChecklistItem() != null
                    && mDashboardViewModel.getMyAssignedChecklistItem().getChecklistStatus().equals("In Progress")) {
                openChecklistDetailScreen(mDashboardViewModel.getMyAssignedChecklistItem().getChecklistId(), mDashboardViewModel.getMyAssignedChecklistItem().getUuid(), mDashboardViewModel.getMyAssignedChecklistItem().getTitle(), mDashboardViewModel.getMyAssignedChecklistItem().getIsSequential());
            } else
                continueChecklist(mDashboardViewModel.getMyAssignedChecklistItem());
        }
    }


    private void openPlaceholderList(MyAssignedChecklistItems items) {
        if (getActivity() != null)
            Navigator.launchActivityWithResult(MyAssignedFragment.this, Constants.REQUEST_RESULT_FOR_START_CHECKLIST,
                    PlaceholderCaptureListActivity.setBundleData(getActivity(), items.getUuid(),
                            ...
                            items.getTitle(), getString(R.string.my_assigned_checklist)));
