 ...
 */
public class CompletedChecklistFragment extends ChecklistBaseFragment<FragmentChecklistBinding, DashboardViewModel> {
    private static final String STATUS = "STATUS";
    private DashboardViewModel mDashboardViewModel;
    private FragmentChecklistBinding mBinding;

    public static CompletedChecklistFragment getInstance() {
        CompletedChecklistFragment fragment = new CompletedChecklistFragment();
    ...
    public void onViewCreated(@NonNull View view, Bundle savedInstanceState) {
      ...
      /*  if (mDashboardViewModel.getAllCheckListAdapter().getItemCount() == 0)
        ...
        mBinding.emptyViewText2.setText(getString(R.string.empty_complete_checklist_message));
    }
                                            ...
                                            public void onChanged(@Nullable WorkInfo workStatus) {
                                                if (workStatus != null) {
                                                    if (workStatus.getState() == WorkInfo.State.SUCCEEDED) {
                                                        WorkManager.getInstance(getActivity()).getWorkInfoByIdLiveData(oneTimeWorkRequest.getId()).removeObservers(CompletedChecklistFragment.this);
    ...
    public void onStartObserving() {
        mDashboardViewModel.observeCompletedCheckList().observe(getViewLifecycleOwner(), new Observer<PagedList<CancelledCompletedChecklistItems>>() {
            @Override
            public void onChanged(@Nullable PagedList<CancelledCompletedChecklistItems> completedChecklistItems) {
                mDashboardViewModel.setIsLoading(false);
                mDashboardViewModel.setCompletedChecklistInAdapter(completedChecklistItems);
            }
        });
    }
    ...
    public void removeObserverIfAny() {
        if (mDashboardViewModel.observeCompletedCheckList() != null && mDashboardViewModel.observeCompletedCheckList().hasObservers())
            mDashboardViewModel.observeCompletedCheckList().removeObservers(getViewLifecycleOwner());
    }
