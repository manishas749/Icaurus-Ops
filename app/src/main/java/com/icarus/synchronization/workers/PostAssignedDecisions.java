...
import com.icarus.synchronization.api.AssignedDecisionsApi;
...
public class PostAssignedDecisions extends CommonWorker {
    private final Context mContext;

    public PostAssignedDecisions(@NonNull Context context, @NonNull WorkerParameters params) {
    ...
    public Result doWork() {
        ...
        List<AssignedDecisionEntity> list = postSynchronizationDao.getNonSyncedAssignedDecision();
        if (list.size() > 0) {

            AddUpdateAssignedDecisionsRequest addUpdateAssignedDecisionsRequest = new AddUpdateAssignedDecisionsRequest();
            addUpdateAssignedDecisionsRequest.setData(PostModelMapper.mapAssignedDecision(list));

            RetroUtils.getRetrofitInstance(getApplicationContext(), this).create(AssignedDecisionsApi.class).assignedDecisionAdd(Constants.HEADER_ACCEPT, addUpdateAssignedDecisionsRequest).subscribe(new AbstractNetworkObservable<AddUpdateAssignedDecisionResponse>() {
                @Override
                public void success(AddUpdateAssignedDecisionResponse response) {
                        ...
                        for (int i = 0; i < response.getData().size(); i++) {
                            try {
                                if (response.getData().get(i).getOperation().equalsIgnoreCase(Operation.INSERT.getValue())
                                    ...
                                    postSynchronizationDao.updateSyncStatusAssignedDecision(response.getData().get(i).getUuid());
                                    ...
                                    getSynchronizationDao.insertAssigneDecision(PostModelMapper.mapInsertAssignedDecisionEntity(response.getData().get(i)));
                                }
                            } catch (Exception ex) {
                                ex.printStackTrace();
                            }
