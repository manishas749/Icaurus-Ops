...
public class WorkorderBillingWorker extends CommonWorker {
    private final Context mContext;

    public WorkorderBillingWorker(@NonNull Context context, @NonNull WorkerParameters params) {
    ...
    private void hitAPI(int pageNumber) {
        RetroUtils.getRetrofitInstance(getApplicationContext(), this).create(WorkOrdersApi.class).workorderBillingTypeIndex(Constants.HEADER_ACCEPT, Parameters.PAGE_SIZE, pageNumber, BaseApplication.getPreferenceManager().getLastActivityAfter(), BaseApplication.getPreferenceManager().getLastActivityBefore(), BaseApplication.getPreferenceManager().getRevisionNumber()).subscribe(new AbstractNetworkObservable<RetrieveAllWorkorderBillingTypes>() {

            @Override
            public void success(RetrieveAllWorkorderBillingTypes retrieveAllWorkorderStatuses) {
                if (retrieveAllWorkorderStatuses != null && retrieveAllWorkorderStatuses.getData() != null && retrieveAllWorkorderStatuses.getData().size() > 0) {
                    ...
                    for (int i = 0; i < retrieveAllWorkorderStatuses.getData().size(); i++) {
                        getSynchronizationDao.insertWorkorderBillingType(ModelMapper.mapWorkOrderBillingTypeEntity(retrieveAllWorkorderStatuses.getData().get(i)));
                    }
                }
            }
