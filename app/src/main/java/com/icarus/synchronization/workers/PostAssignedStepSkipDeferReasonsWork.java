...
public class PostAssignedStepSkipDeferReasonsWork extends CommonWorker {
    private final Context mContext;

    public PostAssignedStepSkipDeferReasonsWork(@NonNull Context context, @NonNull WorkerParameters params) {
    ...
    public Result doWork() {
        ...
        final List<AssignedStepEntity> assignedStepSkipDeferList = postSynchronizationDao.getNonSyncedAssignedStepSkipDefer();
        AddUpdateAssignedStepsRequest addUpdateAssignedStepSkipDeferRequest = new AddUpdateAssignedStepsRequest();
        addUpdateAssignedStepSkipDeferRequest.setData(PostModelMapper.mapAssignedStep(assignedStepSkipDeferList));

        if (addUpdateAssignedStepSkipDeferRequest.getData().size() > 0) {
            RetroUtils.getRetrofitInstance(getApplicationContext(), this).create(AssignedStepsApi.class).assignedStepSkipDeferAdd(Constants.HEADER_ACCEPT, addUpdateAssignedStepSkipDeferRequest).subscribe(new AbstractNetworkObservable<AddUpdateAssignedStepsResponse>() {
                @Override
