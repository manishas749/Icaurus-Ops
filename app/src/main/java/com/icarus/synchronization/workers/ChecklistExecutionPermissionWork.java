...
import com.icarus.synchronization.api.PermissionsApi;
...
public class ChecklistExecutionPermissionWork extends CommonWorker {
   final private Context mContext;

    public ChecklistExecutionPermissionWork(@NonNull Context context, @NonNull WorkerParameters params) {
    ...
    private void hitAPI(Integer pageNo) {
        RetroUtils.getRetrofitInstance(getApplicationContext(), this).create(PermissionsApi.class).checklistExecutionPermissionIndex(Constants.HEADER_ACCEPT, Parameters.PAGE_SIZE, pageNo, BaseApplication.getPreferenceManager().getLastActivityAfter(), BaseApplication.getPreferenceManager().getLastActivityBefore(), BaseApplication.getPreferenceManager().getRevisionNumber()).subscribe(new AbstractNetworkObservable<RetreiveChecklistExecutionPermission>() {
            @Override
            public void success(RetreiveChecklistExecutionPermission retreiveChecklistExecutionPermission) {
                if (retreiveChecklistExecutionPermission != null && retreiveChecklistExecutionPermission.getData() != null && retreiveChecklistExecutionPermission.getData().size() > 0) {
                    AppDatabase appDatabase = AppDatabase.getInstance(getApplicationContext());
                    GetSynchronizationDao getSynchronizationDao = appDatabase.getSynchronizationDao();

                    for (int i = 0; i < retreiveChecklistExecutionPermission.getData().size(); i++) {
                        getSynchronizationDao.insertChecklistExecutionPermissions(ModelMapper.mapChecklistExecutionPermissionEntity(retreiveChecklistExecutionPermission.getData().get(i)));
                    }
