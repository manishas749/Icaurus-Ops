...
import com.icarus.synchronization.api.AssignedRoomEquipmentsApi;
...
public class PostAssignedRoomEquipments extends CommonWorker {
    private final Context mContext;

    public PostAssignedRoomEquipments(@NonNull Context context, @NonNull WorkerParameters params) {
    ...
    public Result doWork() {
        ...
        List<AssignRoomEquipmentsEntity> list = postSynchronizationDao.getNonSyncedRoomAssets();
        if (list.size() > 0) {

            AddUpdateAssginedRoomEquipmentsRequest addUpdateAssginedRoomEquipmentsRequest = new AddUpdateAssginedRoomEquipmentsRequest();
            addUpdateAssginedRoomEquipmentsRequest.setData(PostModelMapper.mapRoomEquipments(list));

            RetroUtils.getRetrofitInstance(getApplicationContext(), this).create(AssignedRoomEquipmentsApi.class).assignedRoomEquipmentAdd(Constants.HEADER_ACCEPT, addUpdateAssginedRoomEquipmentsRequest).subscribe(new AbstractNetworkObservable<AddUpdateAssginedRoomEquipmentsResponse>() {
                @Override
                public void success(AddUpdateAssginedRoomEquipmentsResponse response) {
                        ...
                        for (int i = 0; i < response.getData().size(); i++) {
                            try {
                                if (response.getData().get(i).getOperation().equalsIgnoreCase(Operation.INSERT.getValue())
                                    ...
                                    postSynchronizationDao.updateSyncStatusRoomEquipments(response.getData().get(i).getUuid());
                                    ...
                                    getSynchronizationDao.insertAssignedRoomEquipmentEntity(PostModelMapper.mapInsertAssignedRoomEquipmentEntity(response.getData().get(i)));
                                }
                            } catch (Exception ex) {
                                ex.printStackTrace();
                            }
